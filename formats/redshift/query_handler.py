...
from . import all_entities_config
from .db import Session
from formats.redshift import models
...
class RedshiftQueryHandler:
    ...
    def execute(self):
        ...
        count_query = self.session.query(func.count()).select_from(query.subquery())
    ...
    def build_joins(self, entity_class):
        ...
        if self.entity == "institutions":
            query = (
                self.session.query(*columns_to_select)
                .distinct()
                .join(
                    models.Affiliation,
            ...
            query = self.session.query(*columns_to_select)
    ...
    def get_summary(self, query):
        ...
        count, citation_count, open_access_count, mean_fwci, mean_citation_count = self.session.execute(
